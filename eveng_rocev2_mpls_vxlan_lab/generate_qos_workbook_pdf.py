...
MD_PATH = ROOT / "QOS_IPV6_WORKBOOK.md"
PDF_PATH = ROOT / "QOS_IPV6_WORKBOOK.pdf"
...
def build_story(md_text: str):
    ...
    story = []
    story.append(Paragraph("QoS / IPv6 Practice Workbook", title))
    story.append(
        Paragraph(
            "Step-by-step additive exercises for QoS, IPv6, ECN, WRED, strict priority, PFC boundaries, and RDMA regression on the current working EVE topology.",
            body,
        )
    )
    ...
    if IMG_PATH.exists():
        ...
        story.append(
            Paragraph(
                "Current validated working topology used by this workbook. Management cloud wiring remains external and environment-specific.",
                body,
            )
        )
...
def main():
    ...
    doc = SimpleDocTemplate(
        ...
        title="QoS IPv6 Practice Workbook",
